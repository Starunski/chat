...
3. Adjust widget settings either by editing `fieldData` in StreamElements or via URL parameters on your overlay source. Supported parameters mirror the `defaultConfig` keys in `widget.js` and include:
   ...
   - **Header & indicator:** `headercolor`, `headericoncolor`, `headericonshadow`, `indicator`, `indicatorcolor`, `indicatorbg`, `title`, `titleicon`
   ...
   - **Event typography & icons:** `eventlabelcolor`, `eventtextcolor`, `eventiconcolor`, `eventiconbg` (`auto` lets the palette drive the value)
...
Every option is exposed in the widget bundle via `widgetFieldGroups` (see `widget.js`), so StreamElements will surface color pickers, toggles, and inputs automatically once the files are uploaded. You can additionally edit any value through the URL parameters or the Fields panel — use the `auto` keyword for `eventiconcolor`, `eventiconbg`, `eventlabelcolor`, or `eventtextcolor` to fall back to palette-driven defaults.
